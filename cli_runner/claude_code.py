...
class ClaudeCodeRunner:
        ...
        async for line in stream:
            ...
            try:
                ...
                if msg_type in ["system", "assistant"]:
                    ...
                    logger.info(content, extra = {"claude_session":session_id})
                elif msg_type == "result":
                    result = data.get("result", str(data))
                    logger.info(f"Final Message Recieved : {result}", extra = {"claude_session":session_id})
                    return data
                elif msg_type == "user":
                    pass
                else:
                    logger.debug(f"[OTHER] Received unhandled message type: {line}", extra ={"claude_session":session_id})
    ...
    async def _run_claude_instance(self, prompt : str, directory : str, model : CLAUDE_CODE_MODELS = CLAUDE_CODE_MODELS.CLAUDE_SONNET_4) -> dict:
        cmd_args = [
            "claude",
            "-p", prompt,
            "--output-format", "stream-json",
            "--verbose",
            "--allowedTools", ",".join(self._get_allowed_tools()),
        ]
        ...
        if process.returncode != 0:
            raise ClaudeProcessError(f"Claude CLI failed with exit code {process.returncode}")
        if not result:
            raise ClaudeProcessError("Claude CLI finished successfully but produced no final result.")
        
        return result
    
    async def run_claude_code(self, prompt : str, directory : str, model : CLAUDE_CODE_MODELS = CLAUDE_CODE_MODELS.CLAUDE_SONNET_4):
        last_exception = None
        for attempt in  range(self._retries+1):
            try:
                if attempt > 0:
                    wait_time = 2 ** attempt
                    logger.info(f"Retrying in {wait_time} seconds... (Attempt {attempt + 1}/{self._retries + 1})")
                    await asyncio.sleep(wait_time)
                return await self._run_claude_instance(prompt, directory, model)
            except (ClaudeProcessError, OSError) as e:
                last_exception = e
                logger.error(
                    f"Execution failed on attempt {attempt + 1}. Error: {e}")
        
        raise ClaudeProcessError(f"All {self._retries + 1} attempts to run Claude failed.") from last_exception
...
async def main():
    ...
    try:
        ...
        result = await read_only_runner.run_claude_code(prompt=prompt, directory=temp_dir)
        ...
        print("\n--- SETUP FAILED ---")
        print("Error: 'claude' command not found. Please ensure it's installed and in your PATH.")
        print("--------------------")
